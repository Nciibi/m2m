...
|------|-------|--------|----------------------|
| Theme Architecture | Create `ThemeProvider` in `App.tsx`, manual theme toggle (light/dark/system), persist to vault/settings | ❌ Not started | All components render correctly in both themes; no console warnings; toggle persists across restarts |
| Dark‑Theme Styles | Populate `src/styles/dark.css` (or extend tokens) with dark variants for colors, backgrounds, borders, scrollbars. Use CSS variables. Audit all components. | 🟡 Partial (tokens.css has dark as root, theme.css has light override) | Visual regression test shows identical layout; colors meet WCAG‑AA contrast |
...
| Emoji Picker Completion | Replace placeholder reaction UI with pop‑over picker (e.g., `emoji-mart` or lightweight alternative). Allow selecting any Unicode emoji, send via `handleSendReaction`. | 🟡 Partial (6 hardcoded emojis only) | Reactions display as inline emoji badges; picker dismisses on outside click |
