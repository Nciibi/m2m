...
|------|-------------------|---------------------------|
| **ChatView** | Message list, reactions (6 hardcoded emojis), markdown rendering, self‑destruct timer, file‑transfer UI, read‑receipt badge, context menu (edit/delete), connection banner, reconnect button, infinite scroll pagination, retention policy UI, **emoji picker ✅**, **typing indicator ✅**, **message search ✅**, **keyboard shortcuts Ctrl+F/K ✅**, **file progress bars ✅**, **sender labels ✅**, **message status ✅**, **drag‑and‑drop ✅** | Extensive keyboard shortcuts (Ctrl+F→search open works, Ctrl+K→settings works, Ctrl+N→new chat in Hub), message search highlighting |
| **HubView** | Conversation list, basic filter/search, navigation to ChatView, mute/unmute conversations, delete conversation, family tab, nearby tab (LAN/DHT discovery), **conversation sorting ✅**, **favorites ✅**, **archive ✅**, **last‑seen ✅**, **invite countdown/history ✅**, **listening indicator ✅** | Keyboard navigation shortcuts (arrow keys, Delete to archive), conversation folders/tags |
...
| Keyboard Shortcut Framework | Add `useHotkeys` utility. Map: `Ctrl+N` → New conversation, `Ctrl+F` → Search, `Ctrl+K` → Settings, `Ctrl+Enter` → Send (when focused), `Esc` → Back/Close. | ✅ Complete (Ctrl+N in Hub, Ctrl+F → search toggle + Ctrl+K → settings in ChatView, Esc to close search) | Shortcuts work in all views without interfering with text entry |
| Typing Indicator Packet (Backend) | Extend `PacketType` enum in `protocol.rs` with `TypingIndicator`. Add serialization. Handle in `session.rs` and `commands/network.rs`. | ✅ Complete (0x60 TypingIndicator, 0x61 TypingIndicatorClear added to protocol.rs, handled in network.rs with `m2m://typing` event) | Backend emits/receives typing packets; compilation succeeds |
| UI Hook for Typing | In `ChatView`, show banner "User is typing…" under message list when typing packet received. Auto‑hide after 3 s inactivity. | ✅ Complete (typingPeers from ChatContext, typing indicator with animated dots, 3s inactivity auto-clear via send_typing_indicator(false)) | Banner appears only when appropriate, disappears after 3 s |
...
| Task | Scope | Status | Acceptance Criteria |
|------|-------|--------|----------------------|
| Favorites / Mute / Archive | Extend `Conversation` model (Rust + TS) with `is_favorite`, `is_muted`, `archived`. Add UI toggle buttons in `HubView` list items. Update DB queries to filter archived out of default list. | ✅ Complete (star toggle → `toggle_favorite`, archive toggle → `toggle_archive`, DB migration adds columns, ConvListItem carries fields) | Users can favorite, mute, archive; UI reflects state instantly |
| Conversation Sorting | Sort conversations by recency (most recent first). | ✅ Complete (sorted in ChatsTab, favorites first, archived last) | Most recent conversations appear first |
...
| Drag‑and‑Drop File Send | Implement `<DropZone>` component over message input area; on drop, invoke `handleSendFile`. Show preview thumbnail for images. | ✅ Complete (drop zone with dashed border overlay + hint text, triggers handleSendFile) | Dragging a file sends it; UI shows progress bar |
...
|-----------|----------|-------------|
| **M1 – Theme & Shortcut Foundation** | ✅ Complete | Dark‑mode CSS ✅, manual light/dark/system toggle ✅, ThemeContext.tsx ✅, keyboard shortcuts ✅ (Ctrl+N/F/K/Esc) |
| **M2 – Messaging UI Polish** | ✅ ~90% complete | Emoji picker ✅, message status indicators ✅, file transfer progress bars ✅, sender labels ✅, Search bar ✅, typing indicator ✅, ARIA live region ❌ |
| **M3 – Conversation Management** | ✅ ~85% complete | Conversation sorting ✅, last-seen ✅, invite countdown/history ✅, listening indicator ✅, Favorites/archive ✅, drag-and-drop ✅, keyboard nav 🟡 partial |
