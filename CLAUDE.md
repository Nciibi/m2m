# M2M — Wire Tier 2 Discovery Plan
...
| `src/hooks/useM2MState.ts` | Wire discovery state (if needed beyond SettingsContext) |
...
The `CancellationToken` approach is already used in the codebase for the heartbeat worker pattern — it's clean, doesn't require `Arc<AtomicBool>` polling.
...
- **Both ON**: LAN and DHT can run simultaneously — users on the same WiFi find each other via LAN, users across the internet find each other via DHT.
- **Tor + Discovery**: If Tor is ON and user enables DHT, warn that DHT traffic doesn't go through Tor (it's raw TCP to bootstrap nodes). DHT announce bypasses Tor.
...
| Test | What it covers |
|---|---|
| `SettingsContext.test.tsx` — LAN toggle | Calls `set_discovery_config` with `lan_enabled: true` |
| `SettingsContext.test.tsx` — DHT toggle | Calls `set_discovery_config` with `dht_enabled: true` |
| `SettingsContext.test.tsx` — both off | Default state, toggles work independently |
| LAN discovery tests (existing) | Unit tests for packet format, parsing, expiry |
| DHT discovery tests (existing) | Unit tests for message roundtrip, peer expiry |
| New: lifecycle test | Start/stop LAN discovery, verify peer count changes |

---

That's the plan. Core idea: wire up what already exists, keep it optional and OFF by default, follow existing patterns precisely.
