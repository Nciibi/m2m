  ...
  return (
    <div className="flex flex-col h-full gap-lg">
      <div className="flex items-center justify-between shrink-0">
        ...
        <button onClick={() => setShowAdd(true)} className="flex items-center gap-1 px-md py-1.5 bg-white/5 border border-white/10 hover:bg-white/10 text-on-surface rounded-lg transition-all">
      ...
      <div className="relative shrink-0">
        ...
        <input value={search} onChange={e => setSearch(e.target.value)} className="w-full h-[36px] bg-black/40 backdrop-blur-3xl border border-white/10 rounded-lg pl-10 pr-md text-body-md text-white focus:ring-1 focus:ring-primary/50 outline-none transition-all" placeholder="Search family…" type="text"/>
      </div>
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar space-y-sm">
        {family.filter(m => m.nickname.toLowerCase().includes(search.toLowerCase()) || m.public_key_hex.includes(search)).map((m) => {
          ...
          return (
            <div key={m.public_key_hex} className="group inner-glass h-16 p-md rounded-xl flex items-center gap-md hover:bg-white/5 transition-all">
              <div className="relative flex-shrink-0">
                <div className="w-12 h-12 rounded-full bg-gradient-to-br from-[#10b981] to-[#3b82f6] flex items-center justify-center font-bold text-white text-lg">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-body-base text-secondary truncate font-mono-code">{m.public_key_hex.substring(0, 16)}...</p>
              </div>
              
              <div className="flex items-center gap-sm">
                {isExpired ? (
                  <>
                    <button className="px-sm py-1 border border-white/10 rounded-lg hover:bg-white/5 text-sm" onClick={() => { setShowUpdate(m.public_key_hex); setUpdateInvite(""); }}>Renew</button>
                  ...
                  <div className="flex items-center gap-sm">
                    <input className="bg-black/40 border border-white/10 rounded px-2 py-1 text-sm outline-none" placeholder="Paste new invite…" value={updateInvite} onChange={e => setUpdateInvite(e.target.value)} />
                  ...
                  <>
                    <button className="px-md py-1.5 bg-white/5 hover:bg-white/10 border border-white/10 rounded-lg font-bold text-sm text-primary flex items-center gap-1 transition-all" onClick={async () => {
...
function AddFamilyModal({ onClose, onDone }: { onClose: () => void; onDone: () => Promise<void> }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
      <div className="bg-surface border border-white/10 rounded-2xl p-xl shadow-2xl max-w-md w-full animate-in fade-in zoom-in-95 duration-200">
        <div className="flex justify-between items-center mb-xl">
          <h2 className="font-headline-2xl text-headline-2xl font-bold">Add to Family</h2>
          <button onClick={onClose} className="text-text-muted hover:text-white transition-colors p-1 rounded-lg hover:bg-white/5"><span className="material-symbols-outlined">close</span></button>
        </div>
        
        <div className="space-y-md">
          <div className="space-y-xs">
            <label className="text-label-sm text-text-muted uppercase tracking-wider">Peer Key</label>
            <input className="w-full bg-input-bg border border-white/10 rounded-lg px-md py-2 font-mono-code focus:ring-1 focus:ring-primary outline-none" placeholder="Peer public key hex" value={peerKeyHex} onChange={e => setPeerKeyHex(e.target.value)} />
          </div>
          <div className="space-y-xs">
            <label className="text-label-sm text-text-muted uppercase tracking-wider">Nickname</label>
            <input className="w-full bg-input-bg border border-white/10 rounded-lg px-md py-2 focus:ring-1 focus:ring-primary outline-none" placeholder="How you'll know them" value={nickname} onChange={e => setNickname(e.target.value)} />
          </div>
          <div className="space-y-xs">
            <label className="text-label-sm text-text-muted uppercase tracking-wider">Duration</label>
            <select className="w-full bg-input-bg border border-white/10 rounded-lg px-md py-2 focus:ring-1 focus:ring-primary outline-none" value={duration} onChange={e => setDuration(e.target.value)}>
          ...
          {duration === "custom" && (
            <div className="space-y-xs animate-in slide-in-from-top-2 duration-200">
              ...
              <input type="number" min={1} className="w-full bg-input-bg border border-white/10 rounded-lg px-md py-2 focus:ring-1 focus:ring-primary outline-none" value={customDays} onChange={e => setCustomDays(e.target.value)} />
            </div>
          )}
        </div>

        <div className="flex justify-end gap-md mt-xl pt-md border-t border-white/10">
          <button onClick={onClose} className="px-md py-2 rounded-lg font-bold hover:bg-white/5 transition-colors">Cancel</button>
